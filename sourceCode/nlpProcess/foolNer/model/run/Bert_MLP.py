...
from utils.NERmodels import Bert_MLP
...
def test(model=None, test_data_loader=None):
    if model is None:
        model = Bert_MLP.from_pretrained('../res/Bert_MLP/best').to(device)
...
def train(EPOCHS, batch_size, lr, full_fine_tuning=False, resume=False, trainAll=False):
    ...
    if not resume:
        model = Bert_MLP.from_pretrained('../../../../dependencies/bert-base-chinese').to(device)
    else:
        log.info('-----Resume training from ../res/Bert_MLP/best!-----')
        model = Bert_MLP.from_pretrained('../res/Bert_MLP/best').to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        model.save_pretrained('../res/Bert_MLP/last')
        ...
        if val_metrics['f1'] > max_val_f1:
            ...
            model.save_pretrained('../res/Bert_MLP/best')
