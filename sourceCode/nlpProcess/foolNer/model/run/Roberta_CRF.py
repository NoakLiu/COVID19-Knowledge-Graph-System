...
from utils.NERmodels import Roberta_CRF
...
def test(model=None, test_data_loader=None):
    if model is None:
        model = Roberta_CRF.from_pretrained('../res/Roberta_CRF/best').to(device)
...
def train(EPOCHS, batch_size, lr, full_fine_tuning=False, resume=False, trainAll=False):
    ...
    if not resume:
        model = Roberta_CRF.from_pretrained('../../../../dependencies/chinese-roberta-wwm-ext').to(device)
    else:
        log.info('-----Resume training from ../res/Roberta_CRF/best!-----')
        model = Roberta_CRF.from_pretrained('../res/Roberta_CRF/best').to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        model.save_pretrained('../res/Roberta_CRF/last')
        ...
        if val_metrics['f1'] > max_val_f1:
            ...
            model.save_pretrained('../res/Roberta_CRF/best')
