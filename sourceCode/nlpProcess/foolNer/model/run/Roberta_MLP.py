...
from utils.NERmodels import Roberta_MLP
...
def test(model=None, test_data_loader=None):
    if model is None:
        model = Roberta_MLP.from_pretrained('../res/Roberta_MLP/best').to(device)
...
def train(EPOCHS, batch_size, lr, full_fine_tuning=False, resume=False, trainAll=False):
    ...
    if not resume:
        model = Roberta_MLP.from_pretrained('../../../../dependencies/chinese-roberta-wwm-ext').to(device)
    else:
        log.info('-----Resume training from ../res/Roberta_MLP/best!-----')
        model = Roberta_MLP.from_pretrained('../res/Roberta_MLP/best').to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        model.save_pretrained('../res/Roberta_MLP/last')
        ...
        if val_metrics['f1'] > max_val_f1:
            ...
            model.save_pretrained('../res/Roberta_MLP/best')
