...
from utils.NERmodels import Bert_BiLSTM_CRF
...
def test(model=None, test_data_loader=None):
    if model is None:
        model = Bert_BiLSTM_CRF.from_pretrained('../res/Bert_BiLSTM_CRF/best').to(device)

    if test_data_loader is None:
        words, labels = preprocess('../../../../dataset/CLUNER/test.json')
...
def train(EPOCHS, batch_size, lr, full_fine_tuning=False, resume=False, trainAll=False):
    word_train, label_train = preprocess('../../../../dataset/CLUNER/train.json')
    word_val, label_val = preprocess('../../../../dataset/CLUNER/dev.json')
    word_test, label_test = preprocess('../../../../dataset/CLUNER/test.json')
    ...
    if not resume:
        model = Bert_BiLSTM_CRF.from_pretrained('../../../../dependencies/bert-base-chinese').to(device)
    else:
        log.info('-----Resume training from ../res/Bert_BiLSTM_CRF/best!-----')
        model = Bert_BiLSTM_CRF.from_pretrained('../res/Bert_BiLSTM_CRF/best').to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        model.save_pretrained('../res/Bert_BiLSTM_CRF/last')
        ...
        if val_metrics['f1'] > max_val_f1:
            ...
            model.save_pretrained('../res/Bert_BiLSTM_CRF/best')
