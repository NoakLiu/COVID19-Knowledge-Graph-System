...
def train(models, EPOCHS, batch_size, lr, resume=False, trainAll=False):
    ...
    for m in models:
        if m == "Bert_MLP":
            model_list.append(Bert_MLP.from_pretrained('../res/Bert_MLP/best').to(device))
        elif m == "Bert_CRF":
            model_list.append(Bert_CRF.from_pretrained('../res/Bert_CRF/best').to(device))
        elif m == "Bert_BiLSTM_CRF":
            model_list.append(Bert_BiLSTM_CRF.from_pretrained('../res/Bert_BiLSTM_CRF/best').to(device))
        elif m == "Roberta_MLP":
            model_list.append(Roberta_MLP.from_pretrained('../res/Roberta_MLP/best').to(device))
        elif m == "Roberta_CRF":
            model_list.append(Roberta_CRF.from_pretrained('../res/Roberta_CRF/best').to(device))
        elif m == "Roberta_BiLSTM_CRF":
            model_list.append(Roberta_BiLSTM_CRF.from_pretrained('../res/Roberta_BiLSTM_CRF/best').to(device))
