...
from utils.NERmodels import Bert_MLP, Bert_BiLSTM_CRF, Roberta_MLP, Roberta_BiLSTM_CRF, Bert_CRF, Roberta_CRF, Ensemble
...
def vote(models):
    ...
    with open('../../../nlpRes/results_.txt', 'w', encoding='utf-8') as f:
        for i in range(len(pred_tags)):
            for j in range(len(pred_tags[i])):
                f.write("{} {} {}\n".format(j, indiv_words[i][j], pred_tags[i][j]))

    with open('../../../nlpRes/entities.txt', 'w', encoding='utf-8') as f:
        for i in range(len(pred_tags)):
            data = {}
                ...
                if pred_tags[i][j].startswith('I-') and pred_tags[i][j - 1].startswith('B-'):
                    ...
                    for k in range(j + 1, len(pred_tags)):
                        if pred_tags[i][k].startswith('I-'):
                            ...
                            break
                    entities.append([start_pos, start_pos + length, pred_tags[i][j][2:], indiv_words[i][start_pos: start_pos + length]])
